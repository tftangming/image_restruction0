...
mesh_transform (TriangleMesh::Ptr mesh, math::Matrix4f const& trans)
{
    ...
    foreach_hmatrix_mult<float, 4> vfunc(trans, 1.0f);//mat = trans, val = 1.0
    foreach_hmatrix_mult<float, 4> nfunc(trans, 0.0f);//mat = trans, val = 0.0
    std::for_each(verts.begin(), verts.end(), vfunc);//对于三维向量的顶点位置，将其转化成齐次向量
