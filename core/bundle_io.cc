...
Bundle::Ptr
load_bundler_ps_intern (std::string const& filename, BundleFormat format)// filename = result1/synth_0.out
{
    std::ifstream in(filename.c_str());
    if (!in.good())
        throw util::FileException(filename, std::strerror(errno));
    ...
    // Reading Photosynther file(~ cameras, ~ features)...
    ...
    std::cout << "success to load the camera parameters" << std::endl;
    /* Read all features. */
    Bundle::Features& features = bundle->get_features();
    features.reserve(num_features);
    for (int i = 0; i < num_features; ++i)
    ...
    std::cout << "success to load the features information" << std::endl;
    in.close();
    return bundle;
}
