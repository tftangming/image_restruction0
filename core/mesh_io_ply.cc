...
load_ply_mesh (std::string const& filename)
{
    ...
    if (!eof && num_grid != 0 && num_grid == grid_rows * grid_cols)
    {
        ...
        core::geom::rangegrid_triangulate(img, mesh);
    }
    ...
    SavePLYOptions const& options)
{
    ...
    bool write_vcolors = options.write_vertex_colors; //默认是true
    write_vcolors = write_vcolors && verts.size() == vcolors.size();
    bool write_vnormals = options.write_vertex_normals;//默认是false
    write_vnormals = write_vnormals && verts.size() == vnormals.size();
    bool write_vconfidences = options.write_vertex_confidences;//默认是true
    write_vconfidences = write_vconfidences && verts.size() == conf.size();
    bool write_vvalues = options.write_vertex_values;//默认是true
    write_vvalues = write_vvalues && verts.size() == vvalues.size();
    bool write_fcolors = options.write_face_colors && face_amount;//默认是true
    write_fcolors = write_fcolors && fcolors.size() == face_amount;
    bool write_fnormals = options.write_face_normals && face_amount;//默认是false
    ...
    out << "comment Export generated by libcore" << std::endl;
    ...
    if (options.format_binary)//默认是true
    {
        ...
        for (std::size_t i = 0; i < verts.size(); ++i)
        {
            ...
            if (write_vcolors)
            {
                ...
                ply_color_convert(*vcolors[i], color);//原来的颜色向量元素属于0~1,且是四维向量，现在需要选择前三项元素并调整到正常颜色值
        ...
        unsigned char verts_per_simplex_uchar = options.verts_per_simplex;//每个面片的顶点数目为3
    ...
    ByteImage::ConstPtr color_image)
{
    ...
    camera.fill_inverse_calibration(*invproj, width, height);//invproj指向的矩阵表示从像素坐标投影到相机坐标系上
    ...
    for (int i = 0; i < num_pixels; ++i)
    {
        ...
        if (confidence_map != nullptr)
        {
            confidence = confidence_map->at(x, yinv, 0);//取第x列第yinv行的数据
            if (confidence <= 0.0f)
                continue;
        }
        ...
        math::Vec3f pos = core::geom::pixel_3dpos(x, yinv, depth, invproj);
