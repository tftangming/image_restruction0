...
{
    int const img1_width = image->width();
    int const img1_height = image->height();
    core::ByteImage::Ptr ret = core::ByteImage::create(img1_width, img1_height, 3);
    ret->fill(0);
    /* Copy images into output image. */
    unsigned char* out_ptr = ret->begin();
    unsigned char const* img1_ptr = image->begin();
...
//    }
    int num = rand()%11;
    uint8_t* color = color_table[num]; /*{ 255, 255, 0 }*/
