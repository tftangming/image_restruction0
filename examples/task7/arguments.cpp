 ...
 * enum DataTerm {
 ...
 ;**/
    ...
    for (util::ArgResult const* i = args.next_option();
        ...
        break;
        case 'd':
            conf.settings.data_term = parse_choice<DataTerm>(i->arg);//i->arg是字符串,如"area", "gmi";然后设置conf.settings.data_term = 0,1
        break;
        case 's':
            conf.settings.smoothness_term = parse_choice<SmoothnessTerm>(i->arg);//同上
        break;
        case 'o':
            conf.settings.outlier_removal = parse_choice<OutlierRemoval>(i->arg);//同上
        break;
        case '\0':
