...
//由两张图分别计算的特征计算sift特征匹配对应关系，生成经过ransac处理后的准确的特征对应列表
        ...
        , sfm::FeatureSet&feat2)
{
    ...
    /*2.0 利用本征矩阵对数据进行*/
    // 进行特征点坐标归一化，归一化之后坐标中心位于(0,0), 范围[-0.5, 0.5]。坐标归一化有助于保持计算的稳定性
    ...
    ransac_fundamental.estimate(corr_all, &ransac_fundamental_result); //这一步得到了较为准确的基础矩阵
    // 根据记录的inlier列表对特征匹配对进行筛选
                  ...
                  , float f2)
{
    ...
    // 设置相机1的内参矩阵,第一个参数为焦距，后两个为主轴坐标
    pose1->set_k_matrix(f1, 0.0, 0.0);
    // 相机1为参考矩阵，将其旋转矩阵设置成单位矩阵，平移分量设置为零向量
    pose1->init_canonical_form();
    pose2->set_k_matrix(f2, 0.0, 0.0);

    // todo 从本征矩阵中计算本质矩阵 ,E=K2'FK
    ...
    // 利用一对匹配点对从4中相机姿态中找到最合适的(利用姿态对匹配点进行三角化得到三维点，计算三维点的相机坐标，在两个相机的中的z坐标必须为正
...
//    }
    ...
    std::ofstream out("./tmp/test_ba.txt");
    assert(out.is_open());

    /*捆绑调整*/
    std::vector<sfm::ba::Camera> cams(2);//Camera是一个结构体，里面包括focal_length,distortion,translation,rotation
    ...
    std::copy(pose1.R.begin(), pose1.R.end(), cams[0].rotation);//pose.R是3X3的矩阵
    std::fill(cams[0].distortion, cams[0].distortion + 2, 0.0);//用0填充相机畸变系数
    ...
    // 3d 点，pts_3d和p3ds的类型不一样，虽然都是向量，但是前者的向量元素还是向量，后者的向量元素是sfm::ba::Point3D
    ...
    assert(p3ds.size()==corrs.size());//重建的点的数量和准确匹配对的个数一致则不报错

    // 观察点
    std::vector<sfm::ba::Observation> observations;
    for (std::size_t i = 0; i < p3ds.size(); ++i)
    {
        sfm::ba::Observation obs1;//Observation是一个结构体，包含了像素点的位置，camera_id以及point_id.
        obs1.camera_id = 0;
        obs1.point_id = i;
