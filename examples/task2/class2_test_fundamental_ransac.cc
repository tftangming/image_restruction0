 ...
 *       需要的采样次数 M = -----------
 *                          log(1-p^K)
 * 当控制样本个数为8时，采样一次的失败率为a= 1-p^K ,采样M次失败的概率为b=a^M,采样M次成功的概率为c=1-a^M,整理得上式求得M
 * Example: For p = 50%, z = 99%, n = 8: M = log(0.001) / log(0.99609) = 1176.
 * 需要采样1176次从而保证RANSAC的成功率不低于0.99.
 * @return
 */
//给定内点的概率、拟合模型需要的样本个数、预期的采样成功概率，求解RANSAC需要迭代的次数
...
/**
 * \description 给定匹配对和基础矩阵，计算内点的个数,返回的是一个内点index的向量
 * @param matches
 * @param F
 * @return
 */
 //给定匹配对和基础矩阵，检测出内点（满足要求的匹配对）并返回内点index的向量
...
int main(int argc, char *argv[]){
    ...
    if (argc < 2)
    {
        std::cerr << "Syntax: lack the information of correspondence" << std::endl;
        return 1;
    }
    /*加载归一化后的匹配对 */
    ...
    while(getline(in, line)){
        ...
        if(n_line==0){
            int n_corrs = 0;
            stream>> n_corrs;
            corr_all.resize(n_corrs); //274组对应点
        ...
        if(n_line>0){
            ...
            stream>>corr_all[n_line-1].p1[0]>>corr_all[n_line-1].p1[1]
                  >>corr_all[n_line-1].p2[0]>>corr_all[n_line-1].p2[1];//将一行的四个数据依次填入到p1,p2两个点的坐标值中
        }
        n_line++;
    }
