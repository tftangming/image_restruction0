...
main (int argc, char** argv)
{
    ...
    for (std::size_t i = 0; i < views.size(); ++i) {
        ...
        //提取网格模型中的顶点位置向量列表、顶点法线向量列表、顶点颜线向量列表、顶点置信度列表
        ...
        if (conf.with_normals)
            mesh->ensure_normals();//recalc_normals

        /* If confidence is requested, compute it. */  //最终边界的点的置信度会比较低
        if (conf.with_conf) {
            /* Per-vertex confidence down-weighting boundaries. */
            core::geom::depthmap_mesh_confidences(mesh, 4);//迭代四次计算置信度
        }

        /* If scale is requested, compute it. */   //计算每个顶点的尺度值，计算每个顶点到附近顶点的距离和并取平均 x 2.5
