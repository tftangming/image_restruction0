    ...
    float acos_threshold = 0.95f)
{
    ...
    //找到折叠边所在两面片的另外两个顶点，设为v3、v4
    //将折叠边两侧面片的六个点在faces中的序号设置为0
    ...
    //更新v1的邻域面片信息，移除其中的折叠边两侧面，将原属于v2的面片（不包含折叠边两侧的面片）赋给v1
    ...
    //整理vinfos，更新mesh中的顶点列表信息和面片列表信息等
...
clean_mc_mesh (core::TriangleMesh::Ptr mesh, float needle_ratio_thres)
{
    ...
    //计算面片中最短边与次短边的比值，折叠不合适的边（长度比附近的边长度小很多），同时更新网格节点信息和面信息
    num_collapsed += clean_needles(mesh, needle_ratio_thres);
    //筛选出有三条边的顶点，找到最短的那条边和两侧的面片，执行边折叠
