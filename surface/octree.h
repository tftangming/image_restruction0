...
public:
    ...
    public:
        ...
        //将根节点设置为当前节点,(如果根节点有孩子,将它最左边的最底下的孩子节点作为当前节点),并返回当前节点
        Node* first_leaf (void);

        // point to next node
        // 转向同层的下一个兄弟节点
        //如果当前节点有孩子节点，将它孩子节点作为当前节点并返回当前节点；
        //如果当前节点没有孩子节点，执行next_branch(如果当前节点是第8个节点，则返回到它的父亲节点,继续执行next_branch;如果不是第8个节点，则返回它的下一个兄弟节点当做当前节点)
        Node* next_node (void);
        ...
        //找到他的兄弟节点作为当前节点；若它是第8个节点，则回到上一层寻找兄弟节点作为当前节点
        //如果当前节点是第8个节点，则返回到它的父亲节点,继续执行next_branch;如果不是第8个节点，则返回它的下一个兄弟节点当做当前节点
        Node* next_branch (void);

        // point to next leaf
        //简单的说：找到它所在路径之后的叶子节点，如果没有则先调到它的右分支并找到其路径之后的叶子节点
        //查找它的最小的孩子节点（的孩子节点的...），作为当前节点并返回；如果没有孩子节点执行next_branch,继续查找它的最底层孩子节点作为当前节点返回，若无则返回它作为当前节点
        Node* next_leaf (void);

        // descend
        //找到当前节点的第octant个孩子节点作为当前节点返回
        Iterator descend (int octant) const;
        //查找整个八叉树中第level层的第path个节点作为当前节点返回
        Iterator descend (uint8_t level, uint64_t path) const;

        // ascend
        //找到当前节点的父亲节点作为当前节点并返回
