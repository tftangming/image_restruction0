...
GlobalViewSelection::benefitFromView(std::size_t i)
{
    ...
    for (std::size_t k = 0; k < nFeatIDs.size(); ++k) {
        ...
        float plx = parallax(ftPos, refV, tmpV);//refV和tmpV中包含了相机位置camPos

        // 如果视差小于一定阈值(10度），如果视差越小这里计算得到的score也会变小，所以这里会优先选择具有较大的基线/视差
        if (plx < settings.minParallax)
            score *= sqr(plx / 10.f);

        // Resolution compared to reference view
        float mfp = refV->footPrintScaled(ftPos);//计算参考图像的分辨率
        float nfp = tmpV->footPrint(ftPos); //计算临时图像的分辨率
        float ratio = mfp / nfp; //

        // reference view的分辨率要比tmp view 大2倍以上，因为它用的是target_level的缩小后的尺寸对应的投影矩阵
        if (ratio > 2.)
            ratio = 2. / ratio;
        else if (ratio > 1.)
            ratio = 1.;
        score *= ratio; //如果比值越大，分数越小。这里如果分辨率一样的话，那么对score没有影响

        // 这里还需要控制所选择的全局视图之间的视差也不能太小，如果视差小于10°也会减小score
