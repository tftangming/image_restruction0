...
TextureAtlas::TextureAtlas(unsigned int size) :
    ...
    //std::cout << filename << std::endl;//随机产生一个暂存文件夹/tmp/fileJZDHhO,后面这个是可变的
}
...
TextureAtlas::insert(TexturePatch::ConstPtr texture_patch, float vmin, float vmax) {
    ...
    Rect<int> rect(0, 0, width, height);//创建一个矩形框
  //  std::cout << "The width and height of texture_patch are " << width << " and "<< height <<std::endl;
    /**
     * 117 59
     * 15  14
     * 32  22
    ...
    faces.insert(faces.end(), patch_faces.begin(), patch_faces.end());//把texture_patch中的faces全部放入到TextureAtlas的faces中
    ...
    for (std::size_t i = 0; i < patch_faces.size(); ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            texcoords.push_back(texcoord);//把texture_patch中的texcoords经过偏移和归一化全部放入到TextureAtlas的texcoords中
        }
    }
    return true;
}
...
TextureAtlas::apply_edge_padding(void) {
    ...
    for (unsigned int n = 0; n <= padding; ++n) {
        ...
        for (;it != invalid_border_pixels.end(); it++) {
            ...
            /* Calculate new pixel value. */ //利用周围有效的像素值填充这个边界值
            for (int c = 0; c < 3; ++c) {
                float norm = 0.0f;
                float value = 0.0f;
                for (int j = -1; j <= 1; ++j) {
...
TextureAtlas::finalize() {
    ...
    this->bin.reset();
    this->apply_edge_padding();//在图像上无效值周围如果存在有效值,那么将这个无效值变成有效值
    this->validity_mask.reset();
    this->merge_texcoords();//用来合并多个纹理贴图的坐标值

    core::image::save_png_file(this->image, this->filename);
 //   std::cout << "success to save the textureAtlas image " << this->filename <<std::endl; //未找到存储的图像@！！！！
    this->image.reset();
