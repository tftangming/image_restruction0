...
sample_edge(TexturePatch::ConstPtr texture_patch, math::Vec2f p1, math::Vec2f p2) {
    ...
    math::Vec3f color_test1 = texture_patch->get_pixel_value(p1);
   return color_accum.normalized();//p1为待求像素点
//     return color_test1;
}
    ...
    for (std::size_t i = 0; i < adj_verts.size(); ++i) {
        ...
        for (std::size_t j = 0; j < edge_faces.size(); ++j) {
            for(std::size_t k = j + 1; k < edge_faces.size(); ++k) {
                ...
                //face_label1为小标签,face_label2为大标签
                //根据输入的两种标签找到分别属于这两种标签的那两个网格顶点
    ...
    for (MeshEdge const & seam_edge : seam_edges) {
        ...
        for (ProjectedEdgeInfo const & projected_edge_info : projected_edge_infos) {
            ...
            if (texture_patch_label == label1 || texture_patch_label == label2) {
                ...
                //     color1_accum.add(sample_edge(texture_patch, projected_edge_info.p1, projected_edge_info.p2), 1);
                if (texture_patch_label == label2)
                    ...
                    //     color2_accum.add(sample_edge(texture_patch, projected_edge_info.p1, projected_edge_info.p2), 1);
                num_samples++;
            }
        }
...
global_seam_leveling(UniGraph const & graph,
    ...
    std::vector<std::map<std::size_t, std::size_t> > vertlabel2row;//创建每个顶点的 标签行数映射图
    vertlabel2row.resize(num_vertices);

    std::vector<std::vector<std::size_t> > labels;
    labels.resize(num_vertices);//创建每个顶点的 标签(一个顶点可能存在多个标签,因为它可以存在多个面片上,所以这里的标签用向量表示)
    ...
    for (std::size_t i = 0; i < num_vertices; ++i) {
        ...
        //遍历顶点周边的每一个面片,得到这个顶点所有的非0标签（不会重复）
    ...
    std::size_t x_rows = x_row;//表示g的维度
        ...
        for (std::size_t j = 0; j < labels[i].size(); ++j) {
            ...
            for (std::size_t k = 0; k < labels[i].size(); ++k) {
                ...
                if (label1 < label2) {
                    ...
                    std::vector<MeshEdge> seam_edges;
                    //给定输入的两种标签,找到网格上介于这两种标签的缝隙边,若该顶点不位于缝隙上,则跳过;
    ...
    SpMat Temp = A.transpose() * A;
    SpMat C(Temp.rows(), Temp.rows());
    C.setIdentity();
    // SpMat Lhs = A.transpose() * A + Gamma.transpose() * Gamma;
    SpMat Lhs = A.transpose() * A + Gamma.transpose() * Gamma + 0.04 * C;
    //只保留矩阵的下三角元素(Lhs是一个对称矩阵)
    ...
    std::cout << "\tLhs dimensionality: " << Lhs.rows() << " x " << Lhs.cols() << std::endl;//48431 x 48431
    ...
    for (std::size_t channel = 0; channel < 3; ++channel) {
        ...
        Eigen::VectorXf Rhs = SpMat(A.transpose()) * b;// Rhs = AT * b

        /* Solve for x. */
        Eigen::VectorXf x(x_rows);
        x = cg.solve(Rhs);
        //因为系统属于欠约束,所以需要对计算得到的调整量去均值
