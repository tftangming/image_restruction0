...
calculate_data_costs(core::TriangleMesh::ConstPtr mesh, std::vector<TextureView> * texture_views,
    ...
    #pragma omp parallel
    {
        ...
        for (std::uint16_t j = 0; j < texture_views->size(); ++j) {
            ...
            if (settings.data_term == GMI) {
                ...
                texture_view->erode_validity_mask();//腐蚀边界
            }
            ...
            for (std::size_t i = 0; i < faces.size(); i += 3) {
                ...
                math::Vec3f view_to_face_vec = (face_center - view_pos).normalized();//视线方向
                math::Vec3f face_to_view_vec = (view_pos - face_center).normalized();//负视线方向
                ...
                ProjectedFaceInfo info = {j, 0.0f, math::Vec3f(0.0f, 0.0f, 0.0f)};//标签值，投影质量,面片投影均值颜色
    ...
    for (std::uint32_t i = 0; i < static_cast<std::uint32_t>(projected_face_infos.size()); ++i) {
        for (std::size_t j = 0; j < projected_face_infos[i].size(); ++j) {
            ...
            float data_cost = (1.0f - normalized_quality) * MRF_MAX_ENERGYTERM;//10000000
