...
generate_texture_atlases(std::vector<TexturePatch::Ptr> * orig_texture_patches,
    ...
    float vmin, vmax;
    std::tie(vmin, vmax) = calculate_mapping_function(texture_patches);//在这些纹理面片上计算最大最小像素值 //0.0518629 0.821798
    std::cout << vmin <<" " << vmax << std::endl;
    std::cout << "\tSorting texture patches... " << std::flush;
    //按照纹理面片尺寸降序排列,这里并没有排序成功！！！
    texture_patches.sort();
    texture_patches.reverse();
    std::list<TexturePatch::ConstPtr>::iterator sit = texture_patches.begin();
    /**
    for(;sit != texture_patches.end();){
        std::cout << (*sit)->get_size() << std::endl;
        ++ sit;
    }
     **/
 ...
 //   std::cout << "total " << total_num_patches << " texture_patches"<< std::endl;//362
    ...
    #pragma omp single
    {
//总共有 362个 texture_patches
//atlas 1中放置了 168 个 texture_patches, atlas 1的size为256
//atlas 2中放置了 148 个 texture_patches, atlas 2的size为1024
//atlas 3中放置了 38个 texture_patches, atlas 1的size为1024
//atlas 4中放置了 8 个 texture_patches, atlas 2的size为4096
    while (!texture_patches.empty()) {
        unsigned int texture_size = calculate_texture_size(texture_patches);//计算纹理贴图的尺寸
     //   std::cout<< "texture_size is " << texture_size <<std::endl;//256
        texture_atlases->push_back(TextureAtlas::create(texture_size));//根据纹理尺寸创建该纹理贴图时,得到了padding=size>>7==size/128,padding = 2
   ...
   //         std::cout<< "done_patches are" <<  done_patches <<std::endl;
